...
if ( get_option('require_name_email') && !$user->ID ) {
	if ( 6 > strlen($comment_author_email) || '' == $comment_author )
		err( __('Error: please fill the required fields (name, email).','misa') ); // 將 wp_die 改為錯誤提示
	elseif ( !is_email($comment_author_email))
		err( __('Error: please enter a valid email address.','misa') ); // 將 wp_die 改為錯誤提示
}

if ( '' == $comment_content )
	err( __('Error: please type a comment.','misa') ); // 將 wp_die 改為錯誤提示
...
?>
   <li <?php comment_class(); ?> id="li-comment-<?php comment_ID() ?>">
		<article class="commentwrapper comment-live" id="comment-<?php comment_ID(); ?>">
