...
class Misa_Cat_Widget extends WP_Widget {
	function Misa_Cat_Widget() {
		$widget_ops = array( 'classname' => 'custom_cat', 'description' => __( 'Display a custom list of categories using exclude option.', 'misa' ) );
		$this->WP_Widget( 'Misa-Categories', __( 'Misa Categories', 'misa' ), $widget_ops );
	}
	function widget( $args, $instance ) {
		extract( $args );	
		$title = apply_filters('widget_title', empty( $instance['title'] ) ? __( 'Categories', 'misa' ) : $instance['title']);
	...
	function form( $instance ) {
		...
		<label for="<?php echo $this->get_field_id('hierarchical'); ?>"><?php _e( 'Show hierarchy', 'misa' ); ?></label>
        </p>
		<p><label for="<?php echo $this->get_field_id('exclude'); ?>"><?php _e( 'Exclude:', 'misa' ); ?></label> 
		...
		<br />
		<small><?php _e( 'Category IDs, separated by commas.', 'misa' ); ?></small>
		</p>		
	<?php }
}?>