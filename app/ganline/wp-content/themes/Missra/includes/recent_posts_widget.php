...
class Misa_Recent_Posts extends WP_Widget {
	function Misa_Recent_Posts() {
		$widget_ops = array( 'classname' => 'misa_recent_entries', 'description' => __( 'List recent posts with thumbnails from custom categories.', 'misa' ) );
		$this->WP_Widget('misa-recent-posts', __( 'Misa Recent Posts', 'misa' ), $widget_ops);
		$this->alt_option_name = 'misa_recent_entries';		
	...
	function widget($args, $instance) {
		...
		$title = apply_filters('widget_title', empty($instance['title']) ? __( 'Recent Posts', 'misa' ) : $instance['title']);
            ...
            <ul class="<?php echo $list_class; ?>">
				<?php  while ($r->have_posts()) : $r->the_post();
					...
					$format = '<li>%1$s<a href="%2$s" rel="bookmark" title="' . __( '%3$s' ) . '">%3$s</a><br/>
									<span class="recent-post-meta">%4$s %5$s</span>
								</li>';
	...
	function update( $new_instance, $old_instance ) {
		...
		if ( isset($alloptions['misa_recent_entries']) )
		delete_option('misa_recent_entries');		
		return $instance;
	}
