	...
	<?php if ( have_posts() ) : ?>
		...
		<header class="archive-header">
			<h3 class="archive-page-title">
				<?php if ( is_day() ) : ?>
						<?php printf( __( 'Daily Archives: %s', 'misa' ), '<span>' . get_the_date() . '</span>' ); ?>
					<?php elseif ( is_month() ) : ?>
						<?php printf( __( 'Monthly Archives: %s', 'misa' ), '<span>' . get_the_date( _x( 'Y F', 'monthly archives date format', 'misa' ) ) . '</span>' ); ?>
					<?php elseif ( is_year() ) : ?>
						<?php printf( __( 'Yearly Archives: %s', 'misa' ), '<span>' . get_the_date( _x( 'Y', 'yearly archives date format', 'misa' ) ) . '</span>' ); ?>
					<?php else : ?>
						<?php _e( 'Blog Archives', 'misa' ); ?>
					<?php endif; ?>
				</h3>
			</header>
