<?php
/*
Template Name: portfolio-2columnar 
*/
			...
			if( have_posts() ) :
				...
				<ul class="port2">
					...
					?>
						...
						<?php if ( $thumbnail ) { ?><a rel="prettyPhoto[image]" href="<?php echo $thumbnail; ?>"><img src="<?php echo get_template_directory_uri(); ?>/scripts/timthumb.php?src=<?php echo $thumbnail; ?>&amp;w=280&amp;h=180&amp;zc=1&amp;q=100" alt="<?php the_title(); ?>" title="<?php the_title(); ?>"/></a><?php } ?> </div>
<?php if ( !$titles ) {
						?>
                            <div class="port_content">
                                <a class="port_title" href="<?php the_permalink() ?>" rel="bookmark" title="<?php printf( esc_attr__( '%s', 'misa' ), the_title_attribute( 'echo=0' ) ); ?>"><?php the_title(); ?></a>                           
                            </div>   
                        <?php } ?>
                        </li>
                        <?php $counter++; 
